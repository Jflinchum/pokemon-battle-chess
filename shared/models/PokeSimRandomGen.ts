...
export class PokeSimRandomGen {
  ...
  private buildRandomSet = (s: SpeciesName): PokemonSet => {
    ...
    const sets = this.randomSets[s].sets;
		...
		let teraType = this.sampleIfArray(teraTypes) as TypeName;
		...
		// Get moves
		const moves = this.randomMoveset(types, abilities, species, movePool, teraType, role);
      ...
      item = this.getItem(ability, moves, counter, species, role);
		...
		return {
			...
			species: forme,
			gender: species.baseSpecies === 'Greninja' ? 'M' : this.generateRandomGender(species),
  ...
  // Generate random moveset for a given species, role, tera type.
	private randomMoveset(
		...
		if (movePool.includes('trickroom') && role === 'Doubles Wallbreaker') {
			this.addMove('trickroom', moves,
				movePool);
		}

		if (movePool.includes('auroraveil')) {
			this.addMove('auroraveil', moves,
				movePool);
		}
		...
		if (!['Bulky Support', 'Doubles Support'].includes(role)) {
			...
			for (const moveid of movePool) {
				const move = this.dex.moves.get(moveid);
				const moveType = this.getMoveType(move, species, abilities, teraType);
				if (!NO_STAB.includes(moveid) && (move.basePower) && teraType === moveType) {
					stabMoves.push(moveid);
				}
			}
			if (stabMoves.length) {
    ...
    let counter = this.queryMoves(moves, species, teraType, abilities)
		...
		if (!counter.stab.length) {
			...
			for (const moveid of movePool) {
				const move = this.dex.moves.get(moveid);
				const moveType = this.getMoveType(move, species, abilities, teraType);
				if (!NO_STAB.includes(moveid) && (move.basePower) && types.includes(moveType)) {
					stabMoves.push(moveid);
				}
			}
			if (stabMoves.length) {
    ...
    counter = this.queryMoves(moves, species, teraType, abilities)
		...
		if (!counter.damagingMoves.length) {
			...
			for (const moveid of movePool) {
				const move = this.dex.moves.get(moveid);
				if (!NO_STAB.includes(moveid) && (move.category !== 'Status')) attackingMoves.push(moveid);
			}
		...
		while (moves.size < this.maxMoveCount && movePool.length) {
			...
			for (const pair of MOVE_PAIRS) {
				if (moveid === pair[0] && movePool.includes(pair[1])) {
					this.addMove(pair[1], moves,
						movePool);
				}
				if (moveid === pair[1] && movePool.includes(pair[0])) {
					this.addMove(pair[0], moves,
						movePool);
				}
			}
		}
  ...
  getMoveType(move: Move, species: Species, abilities: string[], teraType: TypeName): TypeName {
  ...
  queryMoves(
      ...
      hazards: [],
		...
		for (const moveid of moves) {
			...
			if (move.multihit && Array.isArray(move.multihit) && move.multihit[1] === 5) counter.skilllink.push(move);
			if (move.recoil || move.hasCrashDamage) counter.recoil.push(move);
			if (move.drain) counter.drain.push(move);
			// Moves which have a base power:
			if (move.basePower) {
				if (!NO_STAB.includes(moveid) || PRIORITY_POKEMON.includes(species.id) && move.priority > 0) {
			...
			if (move.accuracy && move.accuracy !== true && move.accuracy < 90) counter.inaccurate.push(move);
  ...
  getAbility(
		...
		if (species.id === 'hitmonchan' && counter.ironfist.length) return 'Iron Fist';
		if ((species.id === 'thundurus' || species.id === 'tornadus') && !counter.physical) return 'Prankster';
		if (species.id === 'toucannon' && counter.skilllink.length) return 'Skill Link';
      ...
      return (role === 'Fast Attacker') ? 'Silver Powder' : 'Life Orb';
		...
		if (['healingwish', 'switcheroo', 'trick'].some(m => moves.has(m))) {
			if (
				...
				role !== 'Wallbreaker' && role !== 'Doubles Wallbreaker' && !counter.priority.length
			) {
				return 'Choice Scarf';
			} else {
		...
		if (counter.status && (species.name === 'Latias' || species.name === 'Latios')) return 'Soul Dew';
		...
		if (ability === 'Magic Guard' || (ability === 'Sheer Force' && counter.sheerforce)) return 'Life Orb';
		if (ability === 'Anger Shell') return this.sample(['Rindo Berry', 'Passho Berry', 'Scope Lens', 'Sitrus Berry']);
		if (counter.skilllink.length && ability !== 'Skill Link' && species.id !== 'breloom') return 'Loaded Dice';
		if (ability === 'Unburden') {
			return (moves.has('closecombat') || moves.has('leafstorm')) ? 'White Herb' : 'Sitrus Berry';
		}
		if (moves.has('shellsmash') && ability !== 'Weak Armor') return 'White Herb';
		if (moves.has('meteorbeam') || (moves.has('electroshot'))) return 'Power Herb';
		if (moves.has('acrobatics') && ability !== 'Protosynthesis') return '';
		if (moves.has('auroraveil') || moves.has('lightscreen') && moves.has('reflect')) return 'Light Clay';
		if (ability === 'Gluttony') return `${this.sample(['Aguav', 'Figy', 'Iapapa', 'Mago', 'Wiki'])} Berry`;
		if (species.id === 'giratina' && moves.has('rest') && !moves.has('sleeptalk')) return 'Leftovers';
		if (
  ...
  getItem(
		...
		if (
			species.id !== 'jirachi' && (counter.physical >= 4) &&
			...
			const scarfReqs = (
				...
				species.baseStats.spe >= 60 && species.baseStats.spe <= 108 &&
				ability !== 'Speed Boost' && !counter.priority.length && !moves.has('aquastep')
			);
			return (scarfReqs && this.random(1, 3) === 1) ? 'Choice Scarf' : 'Choice Band';
		}
		if (
			(counter.special >= 4) ||
			...
			const scarfReqs = (
				...
				species.baseStats.spe >= 60 && species.baseStats.spe <= 108 &&
				ability !== 'Speed Boost' && ability !== 'Tinted Lens' && !moves.has('uturn') && !counter.priority.length
			);
			return (scarfReqs && this.random(1, 3) === 1) ? 'Choice Scarf' : 'Choice Specs';
		}
		if (counter.speedsetup.length && role === 'Bulky Setup') return 'Weakness Policy';
		if (
			!counter.status &&
		...
		if (species.id === 'golem') return (counter.speedsetup.length) ? 'Weakness Policy' : 'Custap Berry';
		...
		if (
			ability === 'Rough Skin' || (
				...
				(species.baseStats.hp + species.baseStats.def) >= 180 && this.random(1, 3) === 1
			) || (
				ability !== 'Regenerator' && !counter.setup.length && counter.recovery.length &&
				this.dex.getEffectiveness('Fighting', species) < 1 &&
				(species.baseStats.hp + species.baseStats.def) > 200 && this.random(1, 3) === 1
			)
		) return 'Rocky Helmet';
		if (moves.has('outrage') && counter.setup.length) return 'Lum Berry';
		if (moves.has('protect') && ability !== 'Speed Boost') return 'Leftovers';
		if (
			role === 'Fast Support' && !counter.recovery.length && !counter.recoil.length &&
			(counter.hazards.length || counter.setup.length) &&
			(species.baseStats.hp + species.baseStats.def + species.baseStats.spd) < 258
		) return 'Focus Sash';
		if (
			!counter.setup.length && ability !== 'Levitate' && this.dex.getEffectiveness('Ground', species) >= 2
		) return 'Air Balloon';
		if (['Bulky Attacker', 'Bulky Support', 'Bulky Setup'].some(m => role === (m))) return 'Leftovers';
		if (species.id === 'pawmot' && moves.has('nuzzle')) return 'Leppa Berry';
		if (role === 'Fast Support' || role === 'Fast Bulky Setup') {
			return (counter.physical + counter.special >= 3 && !moves.has('nuzzle')) ? 'Life Orb' : 'Leftovers';
		}
